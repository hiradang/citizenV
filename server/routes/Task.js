...
router.get('/district', validateToken, async (req, res) => {
  ...
  const [result, metadata] = await db.sequelize
    .query(`select districts.district_name as cityName, districts.id_district as id, 
...
router.get('/ward', validateToken, async (req, res) => {
  ...
  const [result, metadata] = await db.sequelize
    .query(`select wards.ward_name as cityName, wards.id_ward as id, 
...
router.get('/hamlet', validateToken, async (req, res) => {
  ...
  const [result, metadata] = await db.sequelize
    .query(`select hamlets.hamlet_name as cityName, hamlets.id_hamlet as id, 
